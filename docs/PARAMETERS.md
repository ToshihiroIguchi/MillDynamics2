...
| Domain margin | `margin` | – | 0.012 | 0.005 – 0.2 | Fraction of `D` of solid padding outside the shell |
...
| Lifter count | `n_L` | – | 8 | 0 – 32 | 0 = smooth shell (valid case, used by V7) |
| Lifter height | `h_L` | m | 0.025 | 0 – 0.2·R | |
| Lifter width | `w_L` | m | 0.020 | 0 – 0.2·R | |
| Lifter face angle | `α_L` | deg | 0 | −45 – 45 | 0 = radial rectangular bar; non-zero tilts the leading face |

## 2. Operating conditions

| Parameter | Symbol | Unit | Default | Range | Notes |
| --- | --- | --- | --- | --- | --- |
| Speed fraction | `%N_c` | % | 75 | 0 – 200 | Primary speed control |
| Angular velocity | `ω` | rad/s | *derived* | | `ω = 2π · (%N_c/100) · N_c`, sign from direction |
| Rotation direction | – | – | CCW | CCW / CW | |
| Gravity | `g` | m/s² | 9.81 | 0 – 30 | 0 enables the periodic verification cases (V4) |

Derived and displayed read-only: critical speed
`N_c = (1/2π)·sqrt(g/(R − d_p/2))` [rev/s] and [rpm], tip speed `ωR` [m/s].
...
| Charge slip factor | `k_slip` | – | 0.85 | 0 – 1.0 | Bed angular velocity as a fraction of `ω` |
...
| Consistency index | `K` | Pa·sⁿ | 0.5 | 1e-4 – 100 | |
| Flow behaviour index | `n` | – | 0.7 | 0.2 – 2.0 | `n = 1` ⇒ Newtonian/Bingham; `n < 1` shear-thinning; `n > 1` shear-thickening |
| Yield stress | `τ_y` | Pa | 5 | 0 – 500 | `0` ⇒ no yield surface |
| Regularization parameter | `m` | s | 1000 | 10 – 1e5 | **Numerical**, not physical — see E7 |
| Minimum viscosity clamp | `μ_min` | Pa·s | 1e-4 | 1e-6 – 1 | **Numerical** |
| Maximum viscosity clamp | `μ_max` | Pa·s | 1e3 | 1 – 1e6 | **Numerical** — governs the stiffness of unyielded regions |
...
| Mode | Effect |
| --- | --- |
| Newtonian | `n = 1`, `τ_y = 0`; only `K` (= `μ`) remains meaningful |
| Power-law | `τ_y = 0` |
| Bingham | `n = 1` |
| Herschel–Bulkley | all four free (default) |
...
| Advection scheme | – | MacCormack | MacCormack / semi-Lagrangian | The first-order option exists to *demonstrate* the diffusion difference, not as a default |
...
| `%N_c > 100` | "Supercritical: centrifuging. The prescribed charge model assumes cascading (PHYSICS.md A4)." |
